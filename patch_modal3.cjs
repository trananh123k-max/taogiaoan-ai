...
const filterLogic = `  const getLevelFromGrade = (grade: string) => {
...
content = content.replace(/  const getLevelFromGrade = [\s\S]*?  const filteredUploadedPPCTs = myUploadedPPCTs\.filter\([\s\S]*?  \);/m, filterLogic);
