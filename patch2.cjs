...
const tabButtons = `
          ...
          >
            ...
            <span>Kho PPCT ({myUploadedPPCTs.length})</span>
          </button>
          ...
          >
            ...
            <span>+ Tải PPCT & Phân Tích (Admin)</span>
          </button>
`;

code = code.replace("<span>+ Tải Sách & AI Quét Mục Lục (Admin)</span>\n          </button>", "<span>+ Tải Sách & AI Quét Mục Lục (Admin)</span>\n          </button>\n" + tabButtons);

// Render PPCT List Tab
const ppctListTab = `
...
code = code.replace("{/* TAB 2: TẢI SÁCH MỚI (ADMIN) */}", ppctListTab + "\n        {/* TAB 2: TẢI SÁCH MỚI (ADMIN) */}");
